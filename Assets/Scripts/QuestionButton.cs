...
public class QuestionButton : MonoBehaviour
{
    ...
    public void SetState(bool isCurrent, bool isAnswered)
    {
        ...
        if (manager != null && manager.isReviewMode)
        {
            // Trong chế độ xem lại (Review Mode):
            // - Trả lời đúng: Màu xanh lá cây (#1FA659)
            // - Trả lời sai hoặc chưa trả lời: Màu đỏ (#D93838)
            bool isCorrect = manager.IsQuestionCorrect(questionIndex);
            Color resultColor = isCorrect ? new Color(0.12f, 0.65f, 0.35f) : new Color(0.85f, 0.22f, 0.22f);

            if (img != null) img.color = resultColor;
            if (txt != null)
            {
                txt.color = Color.white;
                if (isCurrent)
                {
                    txt.text = $"[{questionIndex + 1}]";
                }
                else
                {
